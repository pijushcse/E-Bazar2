...
public class ManageProductsController implements CleanupControl {
    ...
    class MaintainCatalogTypesActionListener implements ActionListener,
        ...
        public void actionPerformed(ActionEvent e) {
            SessionContext ctx = SessionContext.getInstance();
            Boolean loggedIn = (Boolean) ctx.get(CustomerConstants.LOGGED_IN);
            System.out.println("Maintain Catalog## " + loggedIn);
            if (!loggedIn.booleanValue()) {
                maintainCatalogTypes = new MaintainCatalogTypes();
                LoginControl loginControl = new LoginControl(maintainCatalogTypes, mainFrame, this);
                loginControl.startLogin();
            } else {
                maintainCatalogTypes = new MaintainCatalogTypes();
    ...
    class ShowComboProductListener implements ActionListener {
        public void actionPerformed(ActionEvent evt) {
                maintainProductCatalog.updateModel();
        }
    }            
    ...
    class SaveAddEditCatListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent evt) {
            String tempNetCatName = addEditCatalog.getNewCatalogName();
            IProductSubsystem pss = new ProductSubsystemFacade();
            try { 
                pss.saveNewCatalogName(tempNetCatName);
            ...
            JOptionPane.showMessageDialog(addEditCatalog,
                    "New Catalog Name- " +tempNetCatName+" Added.", "Information",
                    JOptionPane.INFORMATION_MESSAGE);
    ...
    class SaveAddEditProductListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent evt) {
            try {
                String varProductName = addEditProduct.getProductName();
                String varCatalogName = addEditProduct.getProductCatalog();
                String varPriceUnit = addEditProduct.getProductUnitPrice();
                String varMfgDate = addEditProduct.getProductMfgDate();
                String varAvailableQuantity = addEditProduct.getProductAvaiabkeQunatity();
                
                IProductFromGui iProductFromGui = new Product(varProductName, varMfgDate, varAvailableQuantity, varPriceUnit) ;
                
                IProductSubsystem productSubsystem = new ProductSubsystemFacade();
                productSubsystem.saveNewProduct(iProductFromGui, varCatalogName);
                
                JOptionPane.showMessageDialog(addEditProduct, "New product added.", "Information", JOptionPane.INFORMATION_MESSAGE);
            } catch (DatabaseException ex) {
                Logger.getLogger(ManageProductsController.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    ...
    public ActionListener ProductComboBoxListener(MaintainProductCatalog w) {
        return (new ShowComboProductListener());
    }    
