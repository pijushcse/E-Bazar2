...
public class BrowseAndSelectController implements CleanupControl {
    ...
    List<IProductFromDb> productListFromDb = null;
    ...
    class LoginListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            try {
                Boolean status = (Boolean) SessionContext.getInstance().get(CustomerConstants.LOGGED_IN);
                if (status) {
                    SessionContext.getInstance().add(CustomerConstants.LOGGED_IN, Boolean.FALSE);
                    mainFrame.changeLoggedInMenuText("Login");
                    JOptionPane.showMessageDialog(mainFrame, "You logged out.", "Success", JOptionPane.INFORMATION_MESSAGE);
                } else {
                    LoginControl loginControl = new LoginControl(mainFrame, mainFrame);
                    loginControl.startLogin();
                    mainFrame.changeLoggedInMenuText("Logout");
                }
            } catch (Exception ex) {
                ex.printStackTrace();
            }
    ...
    class SelectProductListener implements ActionListener {
        ...
        String[] readProductDetailsData(String type) {
            if (useDefaultData) {
                ...
                for (IProductFromDb ipfd : getProductListFromDb()) {
                    if (ipfd.getProductName().equalsIgnoreCase(type)) {
                        setSelectedProduct(ipfd);  
                        String[] tempVar = {ipfd.getProductName(), "" + ipfd.getUnitPrice(), "" + ipfd.getQuantityAvail(), "Excellent knowledge"};
                        return tempVar;
                    }
                }
            }
    ...
    class QuantityOkListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            //rules.runRules();
            int desiredQuantity = Integer.parseInt(quantityWindow.getQuantityDesired()); 
    ...
    public List<IProductFromDb> getProductListFromDb() {
        return productListFromDb;
    }

    public void setProductListFromDb(List<IProductFromDb> productListFromDb) {
        this.productListFromDb = productListFromDb;
    }
    
    public void setSelectedProduct(IProductFromDb product) {
            selectedProductList.add(product);
        }

    public List<IProductFromDb> getSelectedProduct() {
            return selectedProductList;
        }

    public List<Integer> getDesiredQuantity() {
        return desiredQuantity;
    }

    public void setDesiredQuantity(int desiredQuantity) {
        this.desiredQuantity.add(desiredQuantity);
    }
    ...
    private List<IProductFromDb> selectedProductList = new ArrayList<>();
    private List<Integer> desiredQuantity = new ArrayList<>() ; 
