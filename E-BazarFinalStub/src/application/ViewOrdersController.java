    ...
    class ViewOrderDetailsListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            if (selectedRow >= 0) {
                ...
                String selOrderIdAsString = (String) model.getValueAt(selectedRow, 0);
                Integer selOrderId = Integer.parseInt(selOrderIdAsString);
                ...
                try {
                    customer.initializeCustomer(customer.getCustomerProfile().getCustId());
                } catch (DatabaseException ex) {
                    Logger.getLogger(ViewOrdersController.class.getName()).log(Level.SEVERE, null, ex);
                }
                
                List<IOrder> orderHistory = customer.getOrderHistory();
                IOrder selectedOrder = null;
                boolean orderFound = false;
                for (IOrder order : orderHistory) {
                    if (order.getOrderId().equals(selOrderId)) {
                        selectedOrder = order;
                        orderFound = true;
                        break;
                    }
                ...
                if (!orderFound) {
                    JOptionPane.showMessageDialog(selectOrderWindow,"Selected order not found!",ERROR,JOptionPane.ERROR_MESSAGE);
                }
                
                try {
                    List<IOrderItem> orderItems = selectedOrder.getOrderItems(); 
                    List<String[]> displayableData = OrderUtil.makeItemsDisplayable(orderItems);
                    viewOrderDetailsWindow = new ViewOrderDetailsWindow();
                    viewOrderDetailsWindow.updateModel(displayableData);
                    mainFrame.getDesktop().add(viewOrderDetailsWindow);
                    viewOrderDetailsWindow.setVisible(true);
                } catch (DatabaseException e) {
                    JOptionPane.showMessageDialog(selectOrderWindow, "Database Error: Unable to read order items.", ERROR, JOptionPane.ERROR_MESSAGE);
                }

            } else {
                JOptionPane.showMessageDialog(selectOrderWindow, ERROR_MESSAGE,ERROR, JOptionPane.ERROR_MESSAGE);
            }
        }
    }
