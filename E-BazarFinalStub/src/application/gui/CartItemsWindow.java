...
/**
 * Class Description: This window represents a user's shopping cart. Students:
 * See the readdata method for where data is put into the table.
 */
public class CartItemsWindow extends JInternalFrame implements ParentWindow {
    ...
    IShoppingCartSubsystem cartSubsystem =  ShoppingCartSubsystemFacade.getInstance();
    ...
    private void initializeWindow() {
        ...
        setSize(GuiUtil.SCREEN_WIDTH, GuiUtil.SCREEN_HEIGHT + 25);
    ...
    private void updateModel() {
        ...
        if (USE_DEFAULT_DATA) {
            ...
            ArrayList<Integer> desiredQuantityList = (ArrayList) browseControl.getDesiredQuantity();
            ArrayList<IProductFromDb> selectedProduct = (ArrayList) browseControl.getSelectedProduct();

            for (int i = 0; i < selectedProduct.size(); i++) {
                try {
                    IProductFromDb tempIProductFromDb = selectedProduct.get(i);
                    int desiredQuantity = desiredQuantityList.get(i);
                    double totalPrice = Double.parseDouble(tempIProductFromDb.getUnitPrice()) * desiredQuantity;
                    String[] tempVar = {tempIProductFromDb.getProductName(), "" + desiredQuantity, tempIProductFromDb.getUnitPrice(), "" + totalPrice};
                    theData.add(tempVar);
                    
                    cartSubsystem.addCartItem(tempIProductFromDb.getProductName(), ""+desiredQuantity, ""+totalPrice, null);                    
                    selectedProduct.remove(i);
                    desiredQuantityList.remove(i); 
