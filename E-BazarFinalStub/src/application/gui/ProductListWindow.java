...
/**
 * Class Description: This window displays all known products belonging to the
 * selected catalog group. The catalog group is passed into the constructor as a
 * parameter. In the final version of the application, the products displayed
 * should be all products in the database having category group equal to this
 * parameter.
 */
public class ProductListWindow extends JInternalFrame implements ParentWindow {
    ...
    List<IProductFromDb> ipfds = null; 
    ...
    public ProductListWindow(String type) {
        ...
        System.out.println("$$$$$$$ In ProductListWindow() constructor.");
    ...
    private void updateModel() {
        ...
        if (USE_DEFAULT_DATA) {
            ...
            try {
                List<IProductFromDb> ipfds = pss.getProductList(catalogType) ;
                control.setProductListFromDb(ipfds);
                for (IProductFromDb ipfd : ipfds) { 
                    String[] tempVar = {ipfd.getProductName()}; 
                    theData.add(tempVar);
                }

            } catch (DatabaseException ex) {
                Logger.getLogger(ProductListWindow.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        updateModel(theData);
    }
