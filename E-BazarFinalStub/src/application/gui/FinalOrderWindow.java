...
public class FinalOrderWindow extends JInternalFrame implements ParentWindow {
    ...
    private void updateModel() {
        ...
        if (USE_DEFAULT_DATA) {
            ...
            List<ICartItem> liveCartItems = cartSubsystem.getLiveCartItems();
            
            for (ICartItem iCartItem : liveCartItems) {
                double unitPrice = Double.parseDouble(iCartItem.getTotalprice()) / Integer.parseInt(iCartItem.getQuantity()) ; 
                String[] tempVar = {iCartItem.getProductName(),iCartItem.getQuantity(), ""+unitPrice, iCartItem.getTotalprice()};
                theData.add(tempVar);
            }
        }
        updateModel(theData);
