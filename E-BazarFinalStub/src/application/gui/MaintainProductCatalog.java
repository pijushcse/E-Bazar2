 ...
 * particular catalog group. When a catalog group is selected, the table is
 * updated to display the products in this group. The screen provides Add, Edit
 * and Delete buttons for modifying the choices of products.
 */
public class MaintainProductCatalog extends JInternalFrame implements ParentWindow, IComboObserver {
    ...
    private final float[] COL_WIDTH_PROPORTIONS= {0.4f, 0.2f, 0.2f, 0.2f};
    ...
    public void defineComboPanel() {
        ...
        try {
            ...
            for (String[] catName : catalogNames) {
                if(catName.length >=1)
                    catalogTypeCombo.addItem(catName[0]);  
            }
        } catch (DatabaseException ex) {
            Logger.getLogger(MaintainProductCatalog.class.getName()).log(Level.SEVERE, null, ex);
        }
        
        
        catalogTypeCombo.addActionListener(control.ProductComboBoxListener(this)); 
    ...
    public void updateModel() {
        ...
        if (USE_DEFAULT_DATA) {
            ...
            try {
                String catalogType = this.getComboBoxValue();
                IProductSubsystem pss = new ProductSubsystemFacade();
                List<IProductFromDb> productFromDbs = pss.getProductList(catalogType);
                
                for (IProductFromDb product : productFromDbs) {
                    String[] tempProduct = {product.getProductName(), product.getUnitPrice(),product.getMfgDate(),product.getQuantityAvail()};                     
                    theData.add(tempProduct);
                }
            } catch (DatabaseException ex) {
                Logger.getLogger(MaintainProductCatalog.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    public String getComboBoxValue(){
      return (String) catalogTypeCombo.getSelectedItem();
    }
