...
public class LoginControl {
    ...
    private void loadCustomer(Integer custId) throws DatabaseException {
        ...
        customer.getCustomerProfile().setCustId(custId);
    ...
    private void authenticate(Integer id, String pwd) {
        try {
                ...
                if (authenticated) {
                    SessionContext.getInstance().add(CustomerConstants.LOGGED_IN, Boolean.TRUE);
                    loadCustomer(id);
                    JOptionPane.showMessageDialog(loginWindow, "Login successful", "Success", JOptionPane.INFORMATION_MESSAGE);
                } else {
                    throw new UserException("Either id or password is incorrect.");
                }
        } catch (EBazaarException e) {
            JOptionPane.showMessageDialog(loginWindow, "Error: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            loginWindow.setVisible(true);
        }
    }
    //////// event handling code
    ...
    class SubmitListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            System.out.println("CONTROLLER ## " + id + pwd);
            ...
            if (controller != null) {
                ...
                Boolean loggedIn = (Boolean) SessionContext.getInstance().get(CustomerConstants.LOGGED_IN);
                System.out.println("CONTROLLER LOGGEDIN$$ " + loggedIn);
                if (loggedIn == Boolean.TRUE) {
