...
import business.ordersubsystem.DbClassOrder;
...
public class OrderUtil {
     ...
     * computeTotalPrice(List orderItems), used for quickly assessing total
     * price in preparing data for Credit Verif system.
     *
     */
    ...
    public static List<String[]> extractOrderData(List<IOrder> ordersList) {
        ...
        for (IOrder iOrder : ordersList) {
            (new OrderSubsystemFacade(null)).createOrder(iOrder.getOrderId(), iOrder.getOrderDate(), iOrder.getTotalPrice()); 
            String[] orderAsList = {""+iOrder.getOrderId(), iOrder.getOrderDate(), iOrder.getTotalPrice()}; 
            list.add(orderAsList);
        }        
        return list;
    }
